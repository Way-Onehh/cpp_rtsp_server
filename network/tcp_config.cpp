...
void tcp_config::start(void * ptr)
{
    rtsp_server_ptr = ptr;
}

void tcp_config::add_video_sdp(sdp & sdp ,int time_base)
{
    ...
    mdp.attributes.emplace_back("fmtp:96 packetization-mode=1;");
    return ;
}

void tcp_config::add_audio_sdp(sdp & sdp ,int time_base)
{
    auto & mdp =sdp.mediaDescriptions.emplace_back();   
...
std::string tcp_config::Transport(int port1 ,int port2)
{
    return  "RTP/AVP/TCP;unicast;interleaved="+std::to_string(port1)+"-"+std::to_string(port2);
}
...
std::shared_ptr<channel> tcp_config::getchannel(std::initializer_list<int> args)
{
    ...
    if(rtsp_server_ptr)
    {
        auto rtsp_server_ =  ((rtsp_server*) rtsp_server_ptr)->shared_from_this();
       
        ((rtsp_server*)rtsp_server_.get())->factory_signal.create
        (
            {fd,ch_number},
        ...
        ch->on_close.connect
        (
            [rtsp_server_,ch_number](int fd,void * addr)
            {
                ((rtsp_server*)rtsp_server_.get())->factory_signal.remove({fd,ch_number});
            }
        );
    }
    return ch;
