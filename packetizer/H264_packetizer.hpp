...
public:
    ...
    auto operator()() -> std::tuple<std::shared_ptr<uint8_t[]>, size_t,bool> {
        if (!*this) return {nullptr, 0,0};
    ...
    //start_time time_base frame_rate ssrc packet_count octet_count
    auto get_rtcp_args()
    {
        std::lock_guard lg(mtx);
        return std::make_tuple(int(this->sequence_number_) ,this->total_bytes);
    }
private:
    auto create_single_packet() -> std::tuple<std::shared_ptr<uint8_t[]>, size_t,bool> {
        if (!current_nal_ || nal_size_ == 0) return {nullptr, 0,0};
        ...
        has_more_packets_ = false;
        ++sequence_number_;
        total_bytes += (packet_size - sizeof(rtp_header));
        return {std::move(packet), packet_size,true};
    }
 
    auto create_fragmented_packet() -> std::tuple<std::shared_ptr<uint8_t[]>, size_t,bool>  {
        ...
        total_bytes += (packet_size - sizeof(rtp_header));
        return {std::move(packet), packet_size,bool(header->marker)};
    }
};